...
var id = "NaN";
var name = "Vossbles Heroes";
var description = "COMING SOON WOWOWOWOOOWO!";
...
var init = () => {
    ...
    // q3
    {
        ...
        q3 = theory.createUpgrade(0, currency, new FirstFreeCost(new ExponentialCost(1e7, Math.log2(2))));
    ...
    // q4
    {
        let getDesc = (level) => "q_4=" + getQ4(level).toString(0);
        let getInfo = (level) => "q_4=" + getQ4(level).toString(0);
        q4 = theory.createUpgrade(0, currency, new FirstFreeCost(new ExponentialCost(1e20, Math.log2(2))));
        q4.getDescription = (amount) => Utils.getMath(getDesc(q4.level));
...
var tick = (elapsedTime, multiplier) => {
    ...
    currency.value += bonus * vq1 * vq2 * getQ3(q3.level) * getQ4(q4.level) * q * dt;
...
var getPublicationMultiplier = (tau) => tau.isZero ? BigNumber.ONE : tau.pow(BigNumber.from(1.7));
var getPublicationMultiplierFormula = (symbol) => "{" + symbol + "}^{1.7}";
var getTau = () => currency.value.pow(BigNumber.from(0.3));
...
var getC1 = (level) => Utils.getStepwisePowerSum(level, 2, 20, 1);
var getC2 = (level) => BigNumber.TWO.pow(BigNumber.from(level));
var getQ3 = (level) => Utils.getStepwisePowerSum(level, 69420, 25, 0);
var getQ4 = (level) => Utils.getStepwisePowerSum(level, 30, 8, 0);
